...
export const MIDIIndicator = ({ data, small }: MIDIIndicatorType) => {
  ...
  const dots = useMemo(() => {
    ...
    return array.map((val, index) => ({
      ...
      duration: (!!midi.length ? index : array.length - index) / 100
    })).reverse();
  }, [midi]);

  return (
    <Styled.Container>
      { dots.map((val, index) => <Styled.Dots $active={val.active} $duration={val.duration} key={index} $small={small as boolean} />)}
    </Styled.Container>
  )
};
